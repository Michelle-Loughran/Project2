...
namespace CMS.Web.Controllers
{
    ...
    public class UserController : BaseController
    {
        ...
        public UserController(IPatientService svc, IConfiguration config, IMailService mailer)
        {        
            _config = config;
            _mailer = mailer;
            _svc = svc;
        ...
        public async Task<IActionResult> Login([Bind("Email,Password")] LoginViewModel u)
        {
            var user = _svc.Authenticate(u.Email, u.Password);
            // check if login was unsuccessful and add validation errors
            if (user == null)
            {
                ModelState.AddModelError("Email", "Email Address is already in use. Please try another email address. Thank you");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public IActionResult Register([Bind("Firstname,Surname,Email,Password,PasswordConfirm,Role")] RegisterViewModel u)       
        {
           ...
           var user = _svc.AddUser(u.Firstname, u.Surname,u.Email,u.Password, u.Role);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> UpdateProfile(ProfileViewModel m)
        {
